...
STEPS = [
    ...
    "region", "district", "city", "phone", "username",
]
...
T = {
    ...
    "ask_district": ("Tumaningizni kiriting:", "Enter your district:", "Введите ваш район:"),
    "ask_city": (
        "Shahringizni kiriting (majburiy emas):",
    ...
    "f_district": ("Tuman", "District", "Район"),
    "f_city": ("Shahar", "City", "Город"),
...
CREATE TABLE IF NOT EXISTS users(
    ...
    region TEXT, district TEXT, city TEXT, phone TEXT, username TEXT,
...
def loc(u) -> str:
    return ", ".join(x for x in (u["city"], u["district"], u["region"]) if x) or "—"
...
BAD_KEY = {
    ...
    "age": "bad_age", "region": "bad_place", "district": "bad_place",
    "city": "bad_place", "phone": "bad_phone", "username": "bad_username",
}
...
async def h_start(m: Message, state: FSMContext):
    await state.clear()                       # hamma narsa boshidan
...
async def h_friend(c: CallbackQuery, bot: Bot):
    ...
    try:
        await bot.send_message(
            other,
            t(tl, "req_in", name=fullname(me_u), age=me_u["age"], loc=loc(me_u),
